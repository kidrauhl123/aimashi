---
name: alkaka-friend-pet
description: Create, repair, validate, preview, and package Alkaka partner/friend desktop pet spritesheets from character art, screenshots, generated images, or visual references. Use when generating a pet for a stable Alkaka/Hermes partner. The final atlas remains 8x9 for runtime compatibility, but directional and generic running rows are reserved transparent rows and are not generated.
---
...
Create an Alkaka partner/friend desktop pet from a concept, one or more reference images, or both. This skill owns pet-specific prompt planning, animation rows, frame extraction, atlas geometry, QA, previews, and packaging. It delegates visual generation to `$imagegen`.
...
Hard boundary: do not create, draw, tile, warp, mirror, or synthesize pet visuals with local Python/Pillow scripts, SVG, canvas, HTML/CSS, or other code-native art as a substitute for `$imagegen`. For a normal Alkaka friend pet run, expect up to 7 visual generation jobs: 1 base pet plus 6 row-strip jobs (`idle`, `waving`, `jumping`, `failed`, `waiting`, `review`). Rows 1, 2, and 7 are reserved transparent rows for atlas compatibility and must not be generated as running animations.
...
The optional `Alkaka Q版贴纸风` preset uses this local style reference:

```text
assets/alkaka-style-reference.jpg
```

This reference is a style sheet, not a single pet identity. Use it to guide the base character's visual family: polished Q-version anime partner, large expressive eyes, compact full-body chibi proportions, clean dark anime linework, soft cel shading, detailed but readable hair/outfit, one or two identity props, and a cute game/VTuber sticker or character-sheet feel.

This preset does not force the Codex pixel-art house style. Keep the output compact, chroma-key friendly, animation-friendly, and readable inside 192x208 desktop-pet cells, but allow the cleaner anime sticker look from the reference image. Preserve the current atlas contract and state set. Do not reintroduce `running`, `running-right`, `running-left`, Shimeji rows, or full illustration-sheet workflows for this preset.
...
- `Picturing <Pet>'s poses.` Create the Alkaka friend pose rows, starting with `idle` and `review` to confirm the pet still looks consistent.
...
```bash
SKILL_DIR="/absolute/path/to/alkaka-friend-pet"
python "$SKILL_DIR/scripts/prepare_pet_run.py" \
  --pet-name "<Name>" \
  --description "<one sentence>" \
  --reference /absolute/path/to/reference.png \
  --style-reference "$SKILL_DIR/assets/alkaka-style-reference.jpg" \
...
`prepare_pet_run.py` creates row-specific layout guide images under `references/layout-guides/` for the generated Alkaka states only. Row jobs attach the matching guide as a layout-only input so the model can follow the correct frame count, spacing, centering, and safe padding. Treat these guides as invisible construction references: the generated row strip must not include visible boxes, borders, center marks, labels, guide colors, or the guide background.
...
4. Parent spawns subagents for `idle` and `review` first as identity checks.
5. Parent records the selected `idle` and `review` results returned by subagents.
6. Parent spawns subagents for every remaining row image-generation job.
7. Each subagent receives the row prompt and every listed input image path, invokes `$imagegen`, and returns only the selected `$CODEX_HOME/generated_images/.../ig_*.png` source path.
8. Parent alone runs `record_imagegen_result.py`, repair queueing, finalization, QA, and packaging.

Subagent write boundary: do not let subagents edit `imagegen-jobs.json`, copy files into `decoded/`, run `record_imagegen_result.py`, run `finalize_pet_run.py`, or package the pet. This avoids manifest races and keeps provenance checks centralized.
...
- Explicitly remind the subagent that the prompt's transparency and effects rules are mandatory: no detached effects, no wave marks for `waving`, no shadows/dust for `jumping`, and only attached opaque sprite-like tears/smoke/stars when allowed by the state prompt.
...
- Generate every normal visual job with `$imagegen`: base plus all Alkaka friend row strips.
- Treat only the base job as eligible for prompt-only generation; every row job must attach its listed grounding images.
- Do not generate `running-right`, `running-left`, or `running`; those atlas rows are reserved transparent rows.
...
- Row-by-row review confirms the generated animation cycles are complete enough for Alkaka partner display.
