...
TRANSPARENCY_ARTIFACT_RULES = [
    ...
    "Effects are allowed only when they are state-relevant, opaque, hard-edged, sprite-style, fully inside the same frame slot, and physically touching or overlapping the pet silhouette.",
...
ALKAKA_STICKER_STYLE = (
    "Alkaka Q-version partner desktop pet style: use the attached Alkaka reference "
    "sheet as visual-family guidance only, not pet identity. Create a polished cute "
    "anime sticker-like partner with compact full-body chibi proportions, large "
    "expressive eyes, clean dark linework, soft cel shading, readable hair and outfit "
    "design, one or two identity props, and a game/VTuber character-sheet feel. Keep "
    "the result small, animation-friendly, chroma-key friendly, and readable inside "
    "192x208 desktop-pet cells. Avoid painterly rendering, realistic materials, 3D, "
    "glossy app-icon lighting, scene backgrounds, excessive tiny detail, and anything "
    "that will disappear or turn noisy at desktop-pet size."
)
...
def resolved_style_notes(style_contract: str, raw_style_notes: str) -> str:
    style_contract = (style_contract or DIGITAL_PET_STYLE).strip()
    raw_style_notes = raw_style_notes.strip()
    if not raw_style_notes:
        return style_contract
    return f"{style_contract} Additional user style notes: {raw_style_notes}."


def base_pet_prompt(args: argparse.Namespace) -> str:
    pet_notes = args.pet_notes or "the pet shown in the reference image(s)"
    style_notes = resolved_style_notes(args.style_contract, args.style_notes)
...
Use any attached style reference only for visual family, proportions, line quality, rendering, and presentation. Do not copy a character from the style sheet unless the user explicitly supplied that character as the pet reference.
...
Use the attached pet reference image(s), if any, for pet identity and the attached base pet image as the canonical design. Use any attached style reference only for visual family, not identity. Use the attached layout guide image only for frame count, slot spacing, centering, and safe padding. Simplify any high-resolution reference details into the Codex digital pet sprite style. Do not simply copy the still reference pose. Generate distinct animation poses that create a readable cycle.
...
- Keep the rendering sprite-like: chunky silhouette, clean dark outline, controlled palette, flat or cel shading, minimal tiny detail.
...
def make_jobs(
    ...
    copied_style_refs: list[dict[str, object]],
) -> list[dict[str, object]]:
    reference_inputs = [
        {"path": rel(Path(str(ref["copied_path"])), run_dir), "role": "pet reference"}
        for ref in copied_refs
    ]
    style_reference_inputs = [
        {
            "path": rel(Path(str(ref["copied_path"])), run_dir),
            "role": "style reference only; use for visual family, not pet identity",
        }
        for ref in copied_style_refs
    ]
    ...
    jobs: list[dict[str, object]] = [
        {
            ...
            "input_images": [*style_reference_inputs, *reference_inputs],
            "output_path": "decoded/base.png",
            "depends_on": [],
            "generation_skill": "$imagegen",
            "requires_grounded_generation": bool(reference_inputs or style_reference_inputs),
            "allow_prompt_only_generation": not (reference_inputs or style_reference_inputs),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Reference image used only for style family, not pet identity.",
    )
    parser.add_argument("--output-dir", default="")
    parser.add_argument("--pet-notes", default="")
    parser.add_argument("--style-notes", default="")
    parser.add_argument("--style-contract", default=DIGITAL_PET_STYLE)
    parser.add_argument(
        "--chroma-key",
        default="auto",
    ...
    raw_style_reference_paths = [
        Path(raw_path).expanduser().resolve() for raw_path in args.style_reference
    ]
    ...
    copied_style_refs: list[dict[str, object]] = []
    for index, source in enumerate(raw_style_reference_paths, start=1):
        if not source.is_file():
            raise SystemExit(f"style reference not found: {source}")
        suffix = source.suffix.lower() or ".png"
        copied = ref_dir / f"style-reference-{index:02d}{suffix}"
        shutil.copy2(source, copied)
        meta = image_metadata(copied)
        meta["source_path"] = str(source)
        meta["copied_path"] = str(copied)
        copied_style_refs.append(meta)
    ...
    request = {
        ...
        "style_references": copied_style_refs,
    ...
    jobs = {
        ...
        "run_dir": str(run_dir),
        "primary_generation_skill": "$imagegen",
        "jobs": make_jobs(run_dir, copied_refs, copied_style_refs),
    }
